var RENDERER = {
  CELL_SIZE: 15,
  INIT_FORCE_RATE: 0.08,
  SURPLUS: 5,
  NEIGHBORS: [{ x: 0, y: -1 }, { x: 1, y: 0 }, { x: 0, y: 1 }, { x: -1, y: 0 }],
  VELOCITY_RATE: 0.5,
  DELTA_RATE: 0.3,
  ...
  createElements: function() {
    var columnCount = (this.columnCount =
        Math.ceil(this.width / this.CELL_SIZE) + this.SURPLUS),
      rowCount = (this.rowCount =
        Math.ceil(this.height / this.CELL_SIZE) + this.SURPLUS);
    ...
    for (var y = 0; y <= rowCount; y++) {
      for (var x = 0; x <= columnCount; x++) {
        this.points[this.getPointIndex(x, y)] = new POINT(
          x * this.CELL_SIZE,
          y * this.CELL_SIZE
        );
      }
...
CELL.prototype = {
  render: function(context) {
    ...
    for (var i = 0; i < 3; i++) {
      axis.push({
        x: this.points[i].x - this.points[3].x,
        y: this.points[i].y - this.points[3].y
      });
    }
    var rate =
      (Math.abs(axis[0].x * axis[1].y - axis[1].x * axis[0].y) / 2 +
        Math.abs(axis[1].x * axis[2].y - axis[2].x * axis[1].y) / 2) /
      this.renderer.baseSize;

    context.strokeStyle = "hsl(200, 100%, 99%)"; // base color manipulation ///////////////////////////////////////////////////
    context.fillStyle =
      "hsl(" + (220 - 10 * rate) + ", 30%, " + (80 + 15 * rate) + "%)"; // wave color manipulation ////////////////////////////
    context.beginPath();

    for (var i = 0; i < 4; i++) {
      context[i == 0 ? "moveTo" : "lineTo"](
        this.points[i].x - this.renderer.columnOffset,
        this.points[i].y - this.renderer.rowOffset
      );
    }
    context.closePath();
    context.stroke();
